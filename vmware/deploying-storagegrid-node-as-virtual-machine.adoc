...
NOTE: Le mot de passe d'installation temporaire est utilisé uniquement lors de l'installation du nœud. Après qu'un nœud a été ajouté à la grille, vous pouvez y accéder à l'aide de link:../admin/change-node-console-password.html["mot de passe de la console du nœud"], qui est indiqué dans le fichier  `Passwords.txt` du Recovery Package.
