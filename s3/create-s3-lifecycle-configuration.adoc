...
Par exemple, cette requête PutObject a été émise le 22 juin 2020 et place un objet dans l' `testbucket` godet.
...
NOTE: Pour les compartiments avec gestion des versions, le `x-amz-expiration` l'en-tête de réponse s'applique uniquement aux versions actuelles des objets.
