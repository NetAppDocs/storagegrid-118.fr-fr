...
CAUTION: Lorsque vous configurez votre déploiement StorageGRID pour une protection contre les pertes de site, vous devez également prendre en compte les effets de link:data-protection-options-for-ingest.html["options d'ingestion"] et link:../s3/consistency-controls.html["la cohérence"].
