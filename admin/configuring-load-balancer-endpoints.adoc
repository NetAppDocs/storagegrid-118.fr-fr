...
* Si le point final de l'équilibreur de charge est utilisé par link:../admin/manage-s3-select-for-tenant-accounts.html["Locataires S3 pour S3 Select"], Il ne doit pas utiliser les adresses IP ou FQDN de tout noeud bare-Metal. Seules les appliances de services et les nœuds logiciels basés sur VMware sont autorisés pour les terminaux d'équilibrage de charge utilisés pour S3 Select.
