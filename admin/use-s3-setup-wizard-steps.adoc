...
Pour plus d'informations sur cette tâche, reportez-vous à la section link:creating-tenant-account.html["Créer un compte de locataire"] et
...
. Si vous souhaitez ajouter des instructions spécifiques pour les objets appartenant au nouveau locataire ou compartiment, créez une règle et une nouvelle règle. Voir link:../ilm/access-create-ilm-rule-wizard.html["Création d'une règle ILM"] et link:../ilm/ilm-policy-overview.html["Règles ILM : présentation"].
