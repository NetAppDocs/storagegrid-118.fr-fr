...
* Utiliser un S3 link:../s3/head-object.html["Objet principal"] ou link:../s3/get-object.html["GetObject"] demande. La réponse inclut la réponse spécifique à StorageGRID `x-ntap-sg-cgr-replication-status` en-tête de réponse, qui aura l'une des valeurs suivantes :
+
[cols="1a,2a"]
|===
